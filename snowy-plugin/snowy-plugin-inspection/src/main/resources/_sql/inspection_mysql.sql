-- 1. 语音记录表（主表）
CREATE TABLE IF NOT EXISTS `INSU_VOICE_RECORD` (
    `ID` int NOT NULL AUTO_INCREMENT COMMENT '主键',
    `INSU_VOICE_ID` int NOT NULL COMMENT '语音记录ID',
    `VOICE_URL` varchar(500) COMMENT '录音文件URL',
    `IS_TRANSLATED` tinyint(1) DEFAULT '0' COMMENT '翻译状态：0-未翻译，1-已翻译',
    `IS_INSPECTED` tinyint(1) DEFAULT '0' COMMENT '质检状态：0-未质检，1-已质检',
    `UPLOAD_TIME` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '上传时间',
    `TRANSLATE_TIME` datetime DEFAULT NULL COMMENT '翻译完成时间',
    `TASK_ID` varchar(50) COMMENT '任务ID',
    `INSPECTION_TIME` datetime DEFAULT NULL COMMENT '质检完成时间',
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UK_VOICE_ID` (`INSU_VOICE_ID`),
    KEY `IDX_TASK_ID` (`TASK_ID`),
    KEY `IDX_IS_INSPECTED` (`IS_INSPECTED`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='录音记录表';

-- 2. 对话内容表
CREATE TABLE IF NOT EXISTS `INSU_VOICE_DIALOG` (
    `ID` int NOT NULL AUTO_INCREMENT COMMENT '主键',
    `INSU_VOICE_ID` int NOT NULL COMMENT '录音ID',
    `DIALOG_TEXT` text COMMENT '对话文本',
    `ROLE` int COMMENT '角色：1-坐席，2-客户',
    `START_TIME` int COMMENT '开始时间',
    `END_TIME` int COMMENT '结束时间',
    `DIALOG_TEXT_ID` varchar(50) COMMENT '对话文本编号',
    PRIMARY KEY (`ID`),
    KEY `IDX_VOICE_ID` (`INSU_VOICE_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='对话内容表';

-- 3. 查询结果表
CREATE TABLE IF NOT EXISTS `INSU_VOICE_QUERY_RESULT` (
    `ID` int NOT NULL AUTO_INCREMENT COMMENT '主键',
    `INSU_VOICE_ID` int NOT NULL COMMENT '语音记录ID',
    `TASK_ID` varchar(50) COMMENT '任务ID',
    ...
    PRIMARY KEY (`ID`),
    KEY `IDX_VOICE_ID` (`INSU_VOICE_ID`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='查询结果表';
...
('B2', '误解未纠正', '在沟通过程中客户理解产品内容或条款有误的，TSR须正面给客户解释清楚，不能转移话题', 'B'),
('B3', '虚假停售', '以保险产品即将停售为由进行宣传销售，实际并未停售', 'B'),
